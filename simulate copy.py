...
def gpt_actions(env, obs, dialog_queue, dialog4ref_queue, gpt_path, gpt_error, total_cost, model_name):
    ...
    for idx in range(env.num_agents):
        ...
        # Optional perception stage: summarize key signals as a system message
        if USE_PERCEPTION:
            ...
            # print perception into terminal
        ...
        obs_prompt = f'''
                        ...
                        Respond with ONLY a single JSON object and NOTHING ELSE (no code fences, no explanations). The JSON must contain two keys: "work" and "consumption", both numbers between 0 and 1, using a step size of 0.02. Example: {{"work": 0.84, "consumption": 0.62}}
                    '''
        obs_prompt = prettify_document(obs_prompt)
        # print user prompt to terminal
    ...
    if USE_REFLECTION and env.world.timestep%3 == 0 and env.world.timestep > 0:
        # merge a small slice of recent reflection context if available
        merged_prompts = []
        for dialogs, dialog4ref in zip(dialog_queue, dialog4ref_queue):
            merged = list(dialogs)
            # try to bring last reflection assistant message as system emphasis
        ...
        results, cost = get_multiple_completion(merged_prompts, model_name=model_name)

        total_cost += cost
    else:
        results, cost = get_multiple_completion([list(dialogs) for dialogs in dialog_queue], model_name=model_name)
        total_cost += cost
    ...
    if USE_REFLECTION and (env.world.timestep+1)%3 == 0:
        ...
        results, cost = get_multiple_completion([list(dialogs) for dialogs in dialog4ref_queue],
                                        temperature=0, max_tokens=200, model_name=model_name)
...
def complex_actions(env, obs, beta=0.1, gamma=0.1, h=1):
    ...
    for idx in range(env.num_agents):
        ...
        # actions[str(idx)] = [l, c]
        actions[str(idx)] = [work_action, consumption_action]  # ✅ 使用列表格式
    actions['p'] = [0]
    return actions
...
# ✅ 使用 save_dir 参数
    ...
    if not os.path.exists(f'{base_save_path}data/{policy_model_save}'):
        os.makedirs(f'{base_save_path}data/{policy_model_save}')
    if not os.path.exists(f'{base_save_path}figs/{policy_model_save}'):
        os.makedirs(f'{base_save_path}figs/{policy_model_save}')
    for epi in range(env.episode_length):
        if policy_model == 'gpt':
            ...
            for idx in range(env.num_agents):
                ...
                actions[str(idx)] = [l, int(c)]  # ✅ 使用列表格式
