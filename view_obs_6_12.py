...
# 查看 obs_6.pkl 中的 Agent 0
print("查看 obs_6.pkl 中的 Agent 0:")
view_agent0_obs('obs_6.pkl')
...
try:
    ...
    if '0' in obs_6 and '0' in obs_12:
        agent_6 = obs_6['0']
        agent_12 = obs_12['0']
        
        print("关键指标变化:")
        print(f"时间: {agent_6.get('time', 'N/A')} -> {agent_12.get('time', 'N/A')}")
        print(f"技能: {agent_6.get('SimpleLabor-skill', 'N/A'):.3f} -> {agent_12.get('SimpleLabor-skill', 'N/A'):.3f}")
        print(f"财富: {agent_6.get('SimpleSaving-wealth', 'N/A'):.2f} -> {agent_12.get('SimpleSaving-wealth', 'N/A'):.2f}")
        print(f"消费率: {agent_6.get('SimpleConsumption-Consumption Rate', 'N/A'):.2f} -> {agent_12.get('SimpleConsumption-Consumption Rate', 'N/A'):.2f}")
        print(f"价格: {agent_6.get('SimpleConsumption-price', 'N/A'):.3f} -> {agent_12.get('SimpleConsumption-price', 'N/A'):.3f}")
        
        # 计算变化
        wealth_6 = agent_6.get('SimpleSaving-wealth', 0)
        wealth_12 = agent_12.get('SimpleSaving-wealth', 0)
        if wealth_6 != 0:
            wealth_change = ((wealth_12 - wealth_6) / wealth_6) * 100
